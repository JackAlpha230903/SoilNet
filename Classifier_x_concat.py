...
checkpoint_path = ".../SOILNET_IMAGENET_VICREG_1.pth"
...
def extract_features(loader, model, device):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, unit="batch"):
            ...
            _, _, x_concat = model(x_img, x_light)
...
X_train_concat, y_train_reg, y_train_cls = extract_features(train_loader, model, device)
X_test_concat, y_test_reg, y_test_cls = extract_features(test_loader, model, device)

scaler_concat = StandardScaler()
X_train_concat_std = scaler_concat.fit_transform(X_train_concat)
X_test_concat_std = scaler_concat.transform(X_test_concat)
...
for name, (cls_model, reg_model) in tqdm(classifiers.items(), unit="model"):
    ...
    cls_model.fit(X_train_concat_std, y_train_cls)
    y_pred_cls = cls_model.predict(X_test_concat_std)
    ...
    for i, label in enumerate(["SM_0", "SM_20"]):
        if name == "SVM":
            if label == "SM_0":
                reg_model.fit(X_train_concat_std, y_train_reg_sm0_scaled.ravel())
                y_pred_reg_scaled = reg_model.predict(X_test_concat_std).reshape(-1, 1)
                y_pred_reg[:, i] = target_scaler_sm0.inverse_transform(y_pred_reg_scaled).ravel()
            else:
                reg_model.fit(X_train_concat_std, y_train_reg_sm20_scaled.ravel())
                y_pred_reg_scaled = reg_model.predict(X_test_concat_std).reshape(-1, 1)
                y_pred_reg[:, i] = target_scaler_sm20.inverse_transform(y_pred_reg_scaled).ravel()
        else:
            reg_model.fit(X_train_concat_std, y_train_reg[:, i])
            y_pred_reg[:, i] = reg_model.predict(X_test_concat_std)
...
print("So sánh hiệu suất (Using x_concat features):")
for name in results:
    print(f"{name}:")
